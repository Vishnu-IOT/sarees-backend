...
async function GetProductById(req, res) {
  try {
    ...
    const productWithFullUrls = addFullImageUrls([product], baseUrl)[0];
...
async function CreateProduct(req, res) {
  ...
  try {
    ...
    const finalCollection =
      collection && ["SAREE", "JEWEL"].includes(collection.toUpperCase())
...
async function UpdateProduct(req, res) {
  ...
  try {
    ...
    let mainImageUrl = toRelativeImageUrl(product.image_url, baseUrl);
